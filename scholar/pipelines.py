...
class ArticlesPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        cursor.execute('insert into articles(keywordContains,title,journalName,abstract,keywords,referenceList,citeByNumber,citeBy,authors,date) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(item['keywordContains'], item['title'], item['journalName'],' '.join(item['abstract'])[1:255],' '.join(item['keywords']),item['referenceList'],item['citeByNumber'],item['citeBy'],' '.join(item['authors']),item['date']))
